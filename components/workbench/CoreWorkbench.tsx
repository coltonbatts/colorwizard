...
/** Painter-first workbench orchestration shared by browser and Tauri shells. */
...
import DesktopWorkbench from '@/components/workbench/DesktopWorkbench'
...
import StudioToolsMenu, { type StudioView } from '@/components/workbench/StudioToolsMenu'
...
const WORKBENCH_VIEWS: readonly WorkbenchView[] = ['sample', 'matches', 'stitch', 'library', 'reference', 'structure', 'surface', 'deck']
...
const DESKTOP_PANEL_META: Record<StudioView, { title: string }> = {
  matches: {
    title: 'Threads',
  },
  library: {
...
function getViewFromUrl(): WorkbenchView {
  if (typeof window === 'undefined') return 'sample'
  const params = new URLSearchParams(window.location.search)
  if (params.get('workflow') === 'embroidery') {
    return params.get('view') === 'stitch' ? 'stitch' : 'matches'
  }
  const tool = params.get('tool') as WorkbenchView | null
  return tool && WORKBENCH_VIEWS.includes(tool) && tool !== 'sample' ? tool : 'sample'
}

function writeViewToUrl(view: WorkbenchView) {
  const url = new URL(window.location.href)
  url.searchParams.delete('tool')
  url.searchParams.delete('workflow')
  url.searchParams.delete('view')
  if (view === 'matches' || view === 'stitch') {
    url.searchParams.set('workflow', 'embroidery')
    url.searchParams.set('view', view === 'matches' ? 'threads' : 'stitch')
  } else if (view !== 'sample') {
...
export default function CoreWorkbench() {
  ...
  const [activeTab, setActiveTab] = useState<WorkbenchView>('sample')
  ...
  useEffect(() => {
    const syncFromUrl = () => setActiveTab(getViewFromUrl())
    syncFromUrl()
  ...
  useEffect(() => {
    if (urlStateReady) writeViewToUrl(activeTab)
  }, [activeTab, urlStateReady])

  // Painter-first keyboard shortcuts remain global and avoid navigating hidden modes.
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Shift+S toggles painter versus lab metrics.
  ...
  const handleResultColorSelect = useCallback((rgb: { r: number; g: number; b: number }) => {
    applySampleColor({
      rgb,
  ...
  }, [applySampleColor, setActiveHighlightColor])

  // Secondary workflows render in the same inspector position without becoming peers of sampling.
      ...
      case 'matches':
        return (
          <MatchesTab
            ...
            onColorSelect={handleResultColorSelect}
          />
        )
      case 'library':
        return (
          <PaintLibraryTab onColorSelect={handleResultColorSelect} />
        )
      case 'stitch':
        return (
          <StitchTab
  ...
  const desktopCanvasMode = (activeTab === 'deck' ? 'sample' : activeTab) as Exclude<TabType, 'deck' | 'mix'>
  const activeDesktopPanel =
    activeTab === 'sample'
      ? null
      : DESKTOP_PANEL_META[activeTab as StudioView]
  ...
  useEffect(() => {
    if (!isMobile) return
    if (!WORKBENCH_VIEWS.includes(activeTab)) {
      setActiveTab('sample')
    }
  }, [activeTab, isMobile])
  ...
  const handleCanvasColorSample = (color: Parameters<typeof setSampledColor>[0]) => {
    if (measureMode && calibration) return
    applySampleColor(color)
  }

  const renderCanvas = (mobile: boolean) => (
    <ErrorBoundary
      fallback={({ resetError }) => <CanvasErrorFallback resetError={resetError} onReset={handleClearImage} />}
    >
      <ImageCanvas
        ref={imageCanvasRef}
        image={image}
        ...
        onColorSample={handleCanvasColorSample}
        ...
        sampledColorHex={mobile && isMobileSampleLayout ? null : sampledColor?.hex ?? null}
        workspaceModeLabel={desktopCanvasMode}
        showHud={false}
        mobileLayout={mobile && isMobileSampleLayout ? 'sample' : 'default'}
  ...
  const studioToolsProps = {
    ...
    onViewChange: (view: StudioView) => setActiveTab(view),
    onReturnToPainter: () => setActiveTab('sample'),
    ...
    onOpenReferencePreview: () => imageCanvasRef.current?.openReferencePreview(),
    onClearWorkspace: () => { void handleClearImage() },
    ...
    >
      {!isMobile ? (
        <DesktopWorkbench
          ...
          frameRef={workspaceFrameRef}
          canvasFrameRef={canvasContainerRef}
          toolbar={image ? (
            <div className="workbench-command-gantry pointer-events-none absolute left-1/2 top-5 z-40 w-full max-w-[calc(100%-2.5rem)] -translate-x-1/2">
              <div className="pointer-events-auto">
                <CompactToolbar
                  ...
                  studioTools={<StudioToolsMenu variant="desktop" {...studioToolsProps} />}
                />
              </div>
            </div>
          ) : null}
          highlightControls={activeHighlightColor ? (
            <div className="pointer-events-none absolute left-6 top-28 z-30"><div className="pointer-events-auto"><HighlightControls /></div></div>
          ) : null}
            ...
            <div className="pointer-events-auto flex min-h-0 min-w-0 flex-1">
              {activeTab === 'sample' ? (
                ...
                <AnimatePresence mode="wait">
                  <FloatingInspectorPanel key={activeTab} title={activeDesktopPanel?.title ?? 'Studio Tool'} layoutMode={desktopLayoutMode} onClose={() => setActiveTab('sample')}>
                    <ErrorBoundary fallback={({ error, resetError }) => <SidebarErrorFallback error={error} resetError={resetError} />} key={activeTab}>
                      {renderTabContent()}
                    </ErrorBoundary>
        ...
        <MobileWorkbench
          ...
          canvasFrameRef={canvasContainerRef}
          header={image ? <MobileHeader onOpenMenu={() => setIsNavOpen(true)} /> : null}
          toolbar={image ? (
          ...
          navigation={<StudioToolsMenu variant="mobile" isOpen={isNavOpen} onOpenChange={setIsNavOpen} {...studioToolsProps} />}
        />
      )}
